...
if(isset($submit)){
$rs1 = mysqli_query($con,"select * from groups where scode = '$jcode'");
if(mysqli_num_rows($rs1)<1){
    header("location:signup.php?code=false");
}else{
    $rs2 = mysqli_query($con,"select * from student where student_username ='$username'");
    if(mysqli_num_rows($rs2)<1){
        $row1 = $rs1->fetch_assoc();
        $groupid= $row1['group_uniquename'];
        $studenttable = $groupid."_students";
        $query1 = "insert into student (student_rollnumber,student_name,student_username,student_password,student_email,student_group) values ('$rollnumber','$name','$username','$password','$email','$groupid')";
        $rs3=mysqli_query($con,$query1)or die("cannot create student");
        $student_id = mysqli_insert_id($con);
        $query2 = "insert into ".$studenttable." (student_id,student_name,student_username,student_email) values ('$student_id','$name','$username','$email')";
        $rs4 = mysqli_query($con,$query2) or die("cannot insert you to group");
...
<html lang="en">
  ...
  <body>
      ...
      <div id="content" class="p-4 p-md-5">
               ...
               <div class="row">
                <div class="col-12">
                    <h2 class="contact-title"> STUDENT SIGNUP</h2>
                </div>
                <div class="col-lg-8">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    <input class="form-control" type="number" name="rollnumber" id="rollnumber" placeholder="roll number" onfocus="this.placeholder = ''" onblur="this.placeholder = 'roll number'" required>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-group">
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <input class="form-control" type="text" name="jcode" id="jcode" placeholder="student joining code" onfocus="this.placeholder = ''" onblur="this.placeholder = 'student joining code'" required>
                                </div>
                            </div>
                        </div>
